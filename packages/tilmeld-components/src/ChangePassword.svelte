...
<script lang="ts">
  ...
  let {
    ...
    [
      k in keyof ComponentProps<typeof Textfield> as `newPassword\$${k}`
    ]?: ComponentProps<typeof Textfield>[k];
  } & {
    [
      k in keyof ComponentProps<typeof Textfield> as `newPassword2\$${k}`
    ]?: ComponentProps<typeof Textfield>[k];
  } & {
    [
      k in keyof ComponentProps<typeof Checkbox> as `revokeTokens\$${k}`
    ]?: ComponentProps<typeof Checkbox>[k];
  } & {
    [
